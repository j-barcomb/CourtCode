...
use crate::models::{
    video::VideoFile,
    tag::{Tag, TagCategory, CodeWindow, CodeButton},
    playlist::Playlist,
    player::{Player, Team},
};
...
impl Database {
    ...
    pub fn insert_video(&self, v: &VideoFile) -> Result<()> {
        ...
        self.conn.execute(
            ...
            params![
                ...
                v.venue, v.notes, v.thumbnail_path,
                v.imported_at.to_rfc3339(), json
            ],
        )?;
        Ok(())
    }
    ...
    pub fn list_videos(&self) -> Result<Vec<VideoFile>> {
        ...
        for r in rows {
            let json = r?;
            if let Ok(v) = serde_json::from_str::<VideoFile>(&json) {
                videos.push(v);
            }
        }
        Ok(videos)
    ...
    pub fn insert_code_window(&self, cw: &CodeWindow) -> Result<()> {
        ...
        self.conn.execute(
            "INSERT INTO code_windows (id, name, description, data_json, created_at) VALUES (?1,?2,?3,?4,?5)",
    ...
    pub fn list_code_windows(&self) -> Result<Vec<CodeWindow>> {
        ...
        let mut windows = vec![];
        for r in rows {
            if let Ok(cw) = serde_json::from_str::<CodeWindow>(&r?) {
                windows.push(cw);
            }
        }
        Ok(windows)
    ...
    pub fn tags_for_video(&self, video_id: &str) -> Result<Vec<Tag>> {
        let mut stmt = self.conn.prepare(
            "SELECT data_json FROM tags WHERE video_id=?1 ORDER BY time_in"
        )?;
        let rows = stmt.query_map([video_id], |row| row.get::<_, String>(0))?;
        let mut tags = vec![];
        for r in rows {
            if let Ok(t) = serde_json::from_str::<Tag>(&r?) {
                tags.push(t);
            }
        }
        Ok(tags)
    ...
    pub fn list_playlists(&self) -> Result<Vec<Playlist>> {
        ...
        for r in rows {
            if let Ok(p) = serde_json::from_str::<Playlist>(&r?) {
                playlists.push(p);
            }
        }
        Ok(playlists)
    ...
    pub fn list_players(&self) -> Result<Vec<Player>> {
        ...
        let mut players = vec![];
        for r in rows {
            if let Ok(p) = serde_json::from_str::<Player>(&r?) {
                players.push(p);
            }
        }
        Ok(players)
